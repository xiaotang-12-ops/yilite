...
**项目类型**: 小型项目
**当前版本**: v1.7.0（内部开发版本）/ v1.1.7（GitHub发布版本）
**最后更新**: 2025-11-14
...
**修复的BUG**:
- ✅ 修复"颜色管理"标签页显示错误组件的零件名称
- ✅ 修复不同组件使用相同 node_name 导致的BOM名称混淆

**问题描述**:
1. **不同组件使用了相同的 node_name**：
   - 第一个组件（叉套组焊）：NAUO1 = "方形板-机加"
   - 第二个组件（斗组焊）：NAUO1 = "斗主板"

2. **`allPartsList` 从所有步骤中查找BOM名称**：
   - 查看第一个组件时，NAUO1 被错误地识别为"斗主板"
   - 导致"颜色管理"标签页显示错误的零件名称
...
- `frontend/src/views/ManualViewer.vue`（第1315-1393行，约79行修改）
  - 添加 `stepsToSearch` 变量，根据当前章节类型过滤步骤
  - 组件装配：只搜索当前组件的步骤
  - 产品总装：搜索所有步骤
...
**问题描述**:
1. **组件装配**：累加前面步骤的 `parts_used` 导致绿色零件越来越多
   - `parts_used` 包含了所有参与步骤的零件（已装配+正在装配）
   - 应该累加 `3d_highlight`（只包含正在装配的零件）
2. **产品总装**：只收集步骤1的 components，导致绿色零件不会增加
   - 应该收集前面所有步骤的 components 和 fasteners

**修复方案**:
- 组件装配：将 `step.parts_used` 改为 `step['3d_highlight']`
- 产品总装：将"只收集步骤1"改为"收集前面所有步骤"

**修改文件**:
- `frontend/src/views/ManualViewer.vue`（第1167-1226行，约60行修改）
  - 组件装配：从 `3d_highlight` 中提取零件
  - 产品总装：遍历前面所有步骤收集 components 和 fasteners
...
```json
{
  ...
  "parts_used": [
    ...
    {"bom_name": "方形板", "node_name": ["NAUO21", "NAUO20"]}       // 当前步骤正在装配
  ],
  "3d_highlight": [
    // 只包含当前步骤正在装配的零件
    "NAUO21", "NAUO20"
...
**技术实现**:
- 数据结构：在每个步骤中增加 `custom_colors` 字段（Record<string, 'yellow' | 'green' | 'gray'>）
- 渲染优先级：`custom_colors` > `3d_highlight` > `assembledMeshes` > 默认灰色
- 零件列表提取：直接从3D模型的mesh遍历，不依赖 `parts_used` 字段
- UI设计：折叠面板分组显示，默认展开灰色零件组

**修改文件**:
- `frontend/src/views/ManualViewer.vue`（约200行新增代码）
  - 第684行: editData 增加 custom_colors 字段
  - 第717行: 增加颜色管理相关响应式数据（searchKeyword, colorFilter, activeGroups）
  - 第1149-1276行: 增加零件列表提取和颜色计算的 computed 属性（约128行）
  - 第1240行: watch(showEditDialog) 中初始化 custom_colors
  - 第1450-1494行: 增加颜色管理相关函数（previewColors, clearCustomColors, resetCustomColors）
  - 第1551-1593行: saveManualData 中保存 custom_colors（约42行）
  - 第2150-2276行: highlightStepParts 中增加 custom_colors 优先级判断（约126行修改）
  - 第604-786行: 增加"颜色管理"标签页UI（约182行）
  - 第798-802行: 导入新图标（Search, Delete, RefreshLeft）
  - 第3353-3393行: 增加颜色管理相关CSS样式（约40行）
