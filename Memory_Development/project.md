...
> **版本号说明**：
> - **内部开发版本**（changelog.md）：v1.7.0 - 记录详细的开发历史和功能迭代
> - **GitHub发布版本**（对外发布）：v1.1.7 - 对外发布的稳定版本
...
**新功能**
- 前端支持 `step_node_states` 覆盖三色高亮（current/done/none），保存时写回后端 `assembly_manual.json`
- 悬浮即显示红色包围框，点击零件直接弹状态选择对话框；拖拽旋转不受影响
- 抽屉关闭且未保存时自动回滚覆盖，避免误改

**修改文件**
- `frontend/src/views/ManualViewer.vue`

**后端**
- 复用 `/api/manual/{taskId}` PUT（`backend/simple_app.py`）写盘逻辑，无需新增后端代码
