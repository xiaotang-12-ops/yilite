...
## v1.7.0 (2025-11-21) - 3D 点选自定义高亮 & 悬浮选中框

### 🎯 核心功能

- ✅ 支持用户为每个步骤叠加 `step_node_states`（current / done / none）覆盖色，并保存到后端 `assembly_manual.json`
- ✅ 3D 模型悬浮即显示红色包围框，明确当前命中的零件
- ✅ 左键点击零件直接弹出状态选择对话框（非拖拽时），无需额外开关
- ✅ 关闭高亮抽屉且未保存时自动回滚覆盖，避免误改

### 📝 修改文件

**`frontend/src/views/ManualViewer.vue`**
- 新增 `stepNodeStateMap` 管道：加载/保存时同步 `step_node_states`，PUT `/api/manual/{taskId}` 后端落盘，localStorage 缓存刷新。
- 渲染逻辑：覆盖优先的三色渲染（current/done/none），悬浮 BoxHelper 红框高亮，点击命中零件弹出状态对话框；拖拽旋转不拦截。
- 交互改进：抽屉关闭未保存时回滚；默认开启拾取（无单独开关）；列表/点击保持高亮同步。

**后端说明**
- 复用已存在的 `/api/manual/{taskId}` PUT （`backend/simple_app.py` 约 582 行）写回 `output/{task_id}/assembly_manual.json`，无需新增后端逻辑。

### 🔍 预期效果

- 悬浮就能看到命中的零件（红框），点击零件立刻可改色；拖拽旋转不弹窗。
- “保存更改”后刷新仍保留覆盖色；未保存关闭抽屉则恢复原状态。

### ⚠️ 注意
- 未点击“保存更改”前的覆盖仅存在当前页面，会随刷新/关闭丢失。
- 多人同时编辑时仍无冲突合并，后保存者覆盖前保存者。
...
2. **修改点2：默认标签页（第638行）**
   - 修改默认标签页为 'description'
   ```typescript
   const editActiveTab = ref('description')  // ⭐ 默认显示"步骤描述"标签页
   ```

3. **修改点3：数据加载逻辑（第1206行）**
   - 在数据加载逻辑中添加 description 的加载
   ```typescript
   // ⭐ 加载当前步骤的描述
...
4. **修改点4：保存逻辑（第1453-1485行）**
   - 在保存逻辑中添加 description 的保存（约30行代码）
   - 使用 `step_id` 精确匹配步骤
   - 同时支持组件装配步骤和产品装配步骤

5. **修改点5：UI标签页（第324-356行）**
   - 添加"步骤描述"标签页UI（约30行代码）
   - 文本框高度10行，字符限制500字符
   - 实时显示字符计数
   - 有提示信息说明描述的用途
...
用户打开编辑对话框
    ↓
watch(showEditDialog) 触发
    ↓
从 currentStepData.description 加载数据到 editData.description
    ↓
用户在文本框中编辑
    ↓
用户点击"保存"按钮
...
根据 step_id 找到对应步骤
    ↓
更新 step.description = editData.value.description
    ↓
调用后端 API 保存
    ↓
更新本地 manualData 和 localStorage
    ↓
页面显示更新
```

**步骤匹配逻辑**:
- 使用 `step_id` 精确匹配，确保修改的是正确的步骤
- 先在 `component_assembly` 中查找
- 如果没找到，再在 `product_assembly` 中查找

**UI设计**:
- 文本框高度：10行（`rows="10"`）
- 字符限制：500字符（`maxlength="500"`）
- 字符计数：显示当前字符数（`show-word-limit`）
- 提示信息：说明描述的用途

**默认行为**:
- 打开编辑对话框时，默认显示"步骤描述"标签页
- 如果当前步骤没有 description，显示空文本框
...
- ✅ **数据安全**：使用 step_id 精确匹配，不会误改其他步骤

---

## v1.5.0 (2025-11-14) - 产品装配重复组件检测与修正
...
## v1.4.0 (2025-11-14) - 用户修改描述字段不影响3D高亮
...
## v1.3.1 (2025-11-13) - 产品装配3D高亮验证与修正
...
## v1.2.1 (2025-11-13) - 组件装配3D高亮验证与修正
...
## v1.1.6 (2025-11-12) - 修复焊接模块组件名称同步问题
